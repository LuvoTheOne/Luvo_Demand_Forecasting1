...
st.title("Luvo Demand Forecasting App")

st.divider()

st.header("Input Features")

price = st.number_input("Price", min_value= 0, value= 50)
discount = st.number_input("Discount (%)", min_value= 0, max_value= 100, value= 10)
invetory_level = st.number_input("Inventory Level", min_value= 0, value= 100)
promotion =st.selectbox("Promotion", [0, 1])
competitor_pricing = st.number_input("Copetitor Price", min_value= 0.0, value= 50.0)
...
input_data = pd.DataFrame({
    "Price": [price],
    "Discount": [discount],
    "Inventory Level": [invetory_level],
    "Promotion": [promotion],
